...
def format_train_data(file_name, input_length, output_length):
    ...
    X = np.zeros((500, input_length, WORD_VECTOR_DIMENSION), dtype=np.float)
    Y = np.zeros((500, output_length, VOCABULARY_SIZE + 2), dtype=np.bool)
    for i, seq in enumerate(src_seq[0:500]):
        for j, word in enumerate(seq):
            X[i, j] = get_word_vector(indices_to_words[word], w2v_model)
    for i, seq in enumerate(dst_seq[0:500]):
        for j, word in enumerate(seq):
            Y[i, j, word] = 1
    return X, Y
...
def train(model, X, Y, model_name):
    ...
    print 'training ...'
    model.fit(X, Y, batch_size=128, nb_epoch=50, verbose=1)
    model.predict(X[0:1])
    print 'save model ...'
